...
usage() {
  cat <<'USAGE'
usage: scripts/scaffold-resource.sh --product zia|zpa|ztw|zidentity --resource NAME --package PKG --type TYPE [--out DIR] [--force]
...
case "$product" in
  ...
  *) fail "--product must be zia, zpa, ztw, or zidentity" ;;
esac

...
// Command tui-demo runs a development-only Bubble Tea demo.
//
// Usage:
//
//	go run ./scripts/tui-demo.go [--color auto|always|never] [--format auto|table|pretty|json|ndjson]
//
...
// command surface. It exists only to evaluate terminal rendering, sizing, and
// exit behavior on the feature/tui integration branch.
package main
...
func main() {
	if err := run(context.Background(), os.Args[1:]); err != nil {
		fmt.Fprintf(os.Stderr, "tui demo: %v\n", err)
		os.Exit(2)
	}
}

func run(ctx context.Context, args []string) error {
	flags := flag.NewFlagSet("tui-demo", flag.ContinueOnError)

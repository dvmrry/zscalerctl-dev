...
| Dump writer stages and validates the complete artifact, refuses unsafe overwrites by default, and `dump --force` only atomically exchanges an empty tree or a complete, fully reconciled prior zscalerctl dump after collection succeeds. Unsupported exchange fails closed. | `go test -mod=vendor ./internal/dump ./internal/cli` tests `TestWriteContextCancellationLeavesDestinationUntouched`, `TestRenameNoReplaceRefusesExistingDirectory`, `TestPrepareOutputDirRejectsManifestSchemaPrefixSpoof`, `TestPrepareOutputDirRejectsPartialDump`, `TestPrepareOutputDirRejectsForeignArtifactFile`, `TestPublishReplacementRejectsPostValidationInsertion`, `TestPublishReplacementFailsClosedWithoutAtomicExchange`, `TestDumpRefusesOverwriteBeforeWritingNewFiles`, `TestDumpForceReplacesExistingDumpDirectory`, and `TestDumpForceDoesNotRemovePreviousDumpWhenCollectionFails`; Windows CI runs `TestPublishContextWindowsPublishesNewDestination` and the force fail-closed regression. |

...
| Advisory static and dependency scanners are enabled without becoming PR gates. | `.github/workflows/codeql.yml`, `.github/workflows/osv.yml`, and `.github/workflows/gosec.yml` run on `main`, schedule, or manual dispatch as applicable. Scanner execution failures fail the workflow; SARIF/code-scanning upload unavailability in dev repositories is treated as repository capability noise and preserved as artifacts where possible. Findings require triage before promotion to blocking gates. |
...
| `completion bash|zsh|fish|powershell` and `help` do not read credential files, construct readers, or contact Zscaler. | `go test -mod=vendor ./internal/cli` tests `TestCompletionScriptsDoNotReadCredentialFilesOrUseReader` and `TestHelpDoesNotReadCredentialFile`. |
...
| Dump writer stages and validates the complete artifact, refuses unsafe overwrites by default, and `dump --force` only atomically exchanges an empty tree or a complete, fully reconciled prior zscalerctl dump after collection succeeds. Unsupported exchange fails closed. POSIX publication also requires a stable operator-owned parent namespace. | `go test -mod=vendor ./internal/dump ./internal/cli` tests `TestWriteContextCancellationLeavesDestinationUntouched`, `TestPublishContextCleansDestinationBeforeSelectingParent`, `TestPublishContextRejectsWritableParentBeforeStaging`, `TestPublishContextRejectsNonStickyWritableAncestor`, `TestPublishContextUsesResolvedParentAfterSymlinkSwap`, `TestRenameNoReplaceRefusesExistingDirectory`, `TestPublishContextForceRejectsManifestSchemaPrefixSpoof`, `TestPublishContextForceRejectsPartialDump`, `TestPublishContextForceRejectsForeignArtifactFile`, `TestPublishReplacementRejectsPostValidationInsertion`, `TestPublishReplacementRejectsSameNameSubstitutionBeforeCleanup`, `TestPublishReplacementRefusesRollbackAfterStagingSubstitution`, `TestPublishReplacementFailsClosedWithoutAtomicExchange`, `TestDumpRefusesOverwriteBeforeWritingNewFiles`, `TestDumpForceReplacesExistingDumpDirectory`, and `TestDumpForceDoesNotRemovePreviousDumpWhenCollectionFails`; Darwin runs parent-ACL permit/deny regressions, and Windows CI runs `TestPublishContextWindowsPublishesNewDestination`, restricted-parent DACL inheritance, and the force fail-closed regression. |

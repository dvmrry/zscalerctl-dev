...
| Dump writer refuses unsafe overwrites by default, and `dump --force` only replaces a prior zscalerctl dump directory after collection succeeds. | `go test -mod=vendor ./internal/cli` tests `TestDumpRefusesOverwriteBeforeWritingNewFiles`, `TestDumpForceReplacesExistingDumpDirectory`, `TestDumpForceRejectsNonDumpDirectory`, and `TestDumpForceDoesNotRemovePreviousDumpWhenCollectionFails`. |

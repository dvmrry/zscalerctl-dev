...
not already exist. Use `--force` only to replace a complete zscalerctl dump
directory from a previous run. Replacement requires atomic directory exchange,
an exact supported manifest schema, reconciled manifest, redaction-report,
resource-shape/count and error metadata, and no foreign artifact files. Windows
can publish a new dump directory atomically but fails closed when replacement of
an existing directory would require a non-atomic backup sequence.

Failed cleanup or successful forced replacement can leave an empty owner-only
`.zscalerctl-discard-*` or `.zscalerctl-cleanup-*` directory beside the output.
These directories contain no dump data. They are intentionally not unlinked:
deleting a public parent entry after an identity check would reintroduce a
same-name substitution race.
...
root or leave the outer quarantine empty.

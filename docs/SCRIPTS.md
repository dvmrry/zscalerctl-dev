...
| `scripts/verify-node-toolchain.sh` | verify | `make verify-node-toolchain`; `.github/workflows/ci.yml` | Keeps CI and release on immutable paths and the shared exact Node pin, requires setup before exact direct consumers in a fixed execution context, disables package-manager caching, and verifies PR-CI/release invocation |

...
| `scripts/test-verify-node-toolchain.sh` | test | `make verify-node-toolchain`; `.github/workflows/ci.yml` | Negative fixtures for stale, overridden, or case-colliding pins; conditional or misplaced setup/consumers; environment redirection; missing CI/release wiring; caching; and comment spoofing |
...
| `scripts/verify-node-toolchain.sh` | verify | `make verify-node-toolchain`; `.github/workflows/ci.yml` | Keeps CI and release on immutable paths and the shared exact Node pin, requires setup before the exact direct consumers, disables package-manager caching, and verifies PR-CI/release invocation |

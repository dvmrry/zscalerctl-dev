...
| `scripts/test-verify-node-toolchain.sh` | test | `make verify-node-toolchain`; `.github/workflows/ci.yml` | Negative fixtures for stale or overridden pins, conditional or misplaced setup, missing CI wiring, caching, and comment spoofing |
...
| `scripts/verify-node-toolchain.sh` | verify | `make verify-node-toolchain`; `.github/workflows/ci.yml` | Keeps CI and release on the shared exact Node pin, requires setup before direct consumers, disables package-manager caching, and verifies its PR-CI invocation |

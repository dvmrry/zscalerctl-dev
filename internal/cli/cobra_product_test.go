...
// ── url-lookup (Cobra path) ───────────────────────────────────────────────────

// TestProductCmd_URLLookup_ReachesRunURLLookup verifies that "zia url-lookup
// example.com" via the Cobra product path calls the URLLookupReader capability.
// This exercises the runProduct dispatch to runURLLookup (line ~899 app.go).
func TestProductCmd_URLLookup_ReachesRunURLLookup(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("App.Run(zia url-lookup via Cobra) error = %v, want nil", err)
	}
